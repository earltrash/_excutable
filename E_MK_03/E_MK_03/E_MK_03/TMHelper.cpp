...
namespace MYTM
{
	...
	D2D1_MATRIX_3X2_F MakeRotationMatrix(FLOAT angle, D2D1_POINT_2F center)
	{
	...
	D2D1_MATRIX_3X2_F MakeScaleMatrix(D2D1_SIZE_F size, D2D1_POINT_2F center)
	{
		D2D1_MATRIX_3X2_F _translateToOrigin = MakeTranslationMatrix(D2D1::Size(-center.x, -center.y));
	...
	void MYTM::MakeMatrixToString(const D2D1_MATRIX_3X2_F& matrix, wchar_t* buffer, size_t bufSize) {
		swprintf_s(
			buffer, bufSize,
			L"%.2f %.2f %.2f\n%.2f %.2f %.2f\n",
			matrix._11, matrix._12, matrix._31,
			matrix._21, matrix._22, matrix._32
		);
	}
