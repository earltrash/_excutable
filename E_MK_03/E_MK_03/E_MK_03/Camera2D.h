...
class UnityCamera : public Camera2DBase //걍 싱글톤으로 만들고 값을 외부에서 설정하는 식으로 해야 할 듯 ?
{
public:
...
public:
   ...
   static UnityCamera& GetCamera()
    {
        static UnityCamera Unitycam;
        return Unitycam;
    }
...
private: //일단 상수값 넣었음. 
    float m_screenWidth = 1600.f;    // 렌더 타겟 너비(px)
    float m_screenHeight = 800.f;   // 렌더 타겟 높이(px)
};